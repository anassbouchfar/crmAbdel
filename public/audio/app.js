function restore(){
  ...
  $("#pause").replaceWith('<a class="btn btn-warning one" id="pause">Pause</a>');
...
$(document).ready(function(){
  ...
  $(document).on("click", "#pause:not(.disabled)", function(){
    if($(this).hasClass("resume")){
      Fr.voice.resume();
      $(this).replaceWith('<a class="btn btn-warning one" id="pause">Pause</a>');
    }else{
      Fr.voice.pause();
      $(this).replaceWith('<a class="btn btn-warning one resume" id="pause">Resume</a>');
    }
  });
  ...
  $(document).on("click", "#save:not(.disabled)", function(){
    var client_id = parseInt ($(this).attr('value'));
    function upload(blob){
      ...
      $.ajax({
        ...
        success: function(result) {
          $('.audioContainer'+client_id).prepend(`<audio style="display: block;margin:10px;margin-left:0px" controls id="audio" src="/play/${result.data}"></audio>`)
        }
      });
    }
    if($(this).parent().data("type") === "mp3"){
